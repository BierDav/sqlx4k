...
object Examples {
    suspend fun runAll(db: ISQLite, repo: Sqlx4kRepository) = coroutineScope {
    ...
    suspend fun exampleBasics(db: QueryExecutor) {
        ...
        db.fetchAll("select datetime('now');").getOrThrow().forEach { println(it) }
    ...
    suspend fun exampleConnectionManagement(db: ISQLite) {
    ...
    suspend fun exampleTransactions(db: ISQLite, repo: Sqlx4kRepository) {
        println("\n=== Transactions ===")
        // Prepare table
        db.execute("delete from sqlx4k;").getOrThrow()
    ...
    suspend fun exampleTransactionContext(db: ISQLite) {
        ...
        suspend fun doExtraBusinessLogic(db: ISQLite): Unit = TransactionContext.withCurrent(db) {
